...
const M: MonadThrow2C<E.URI, DecodeError> &
  Bifunctor2<E.URI> &
...
export const failure = <A = never>(
  actual: unknown,
  message: string
): E.Either<DecodeError, A> => E.left(error(actual, message))
...
export const fromRefinement = <I, A extends I>(
  ...
  expected: string
): Decoder<I, A> => K.fromRefinement(M)(refinement, u => error(u, expected))
...
export const type = <A>(
  ...
  pipe(UnknownRecord as any, compose(fromType(properties)))
...
): Decoder<unknown, Array<A>> => pipe(UnknownArray, compose(fromArray(item)))
...
): Decoder<Record<string, I>, Record<string, A>> =>
