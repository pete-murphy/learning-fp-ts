import { Com, pipe, St, Tree } from "./ssstuff/fp-ts-imports"
...
const [treeWithIndices, totalNodesAtDepths] = pipe(
  exampleTree,
  Tree.extend(node => ({ name: node, forestLength: node.forest.length })),
  Tree.traverseWithIndex(St.Applicative)(
    (ns, a) => (totalNodesAtDepths: Array<number>) => {
      totalNodesAtDepths[ns.length] = (totalNodesAtDepths[ns.length] | 0) + 1
      return [
        {
          name: a.name,
...
const flattenedTreeWithEnoughDataToRender = pipe(
  treeWithIndices,
  Tree.map(a => {
    const z = (a.forestLength | 1) / (totalNodesAtDepths[a.depth + 1] | 1)
    return {
      name: a.name,
      x0: (z * a.indexAtDepth) / totalNodesAtDepths[a.depth],
      x1: (z * (a.indexAtDepth + 1)) / totalNodesAtDepths[a.depth],
