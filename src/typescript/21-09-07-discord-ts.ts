...
const testArray: RNEA.ReadonlyNonEmptyArray<string> = ["someKey", "1"]

const fromStringToInt = O.chain((str: string): O.Option<number> => {
  const parsedNumber = parseInt(str)
  if (parsedNumber === NaN) return O.none
  return O.some(parsedNumber)
})

const keyElem = (arr: RNEA.ReadonlyNonEmptyArray<string>): O.Option<string> =>
  RA.lookup(0)(arr)
const groupElem = (arr: RNEA.ReadonlyNonEmptyArray<string>): O.Option<number> =>
...
// const rez1 = pipe(O.of(makeTuple), O.ap(secondEffectful), O.ap(firstEffectful))
