...
import { Iso } from "./Iso";
import { Index } from "monocle-ts/Ix";
import { Lens } from "monocle-ts/Lens";
import { Optional } from "monocle-ts/Optional";
import { Prism } from "monocle-ts/Prism";
import { Traversal } from "monocle-ts/Traversal";
import { At } from "monocle-ts/At";
...
export const isoAsLens = <S, A>(sa: Iso<S, A>): Lens<S, A> =>
  lens(sa.get, flow(sa.reverseGet, constant));

/** @internal */
export const isoAsPrism = <S, A>(sa: Iso<S, A>): Prism<S, A> =>
  prism(flow(sa.get, O.some), sa.reverseGet);
...
export const lensAsOptional = <S, A>(sa: Lens<S, A>): Optional<S, A> =>
  optional(flow(sa.get, O.some), sa.set);
...
/** @internal */
export const prismComposePrism =
  <A, B>(ab: Prism<A, B>) =>
...
export const lensProps =
  <A, P extends keyof A>(...props: readonly [P, P, ...ReadonlyArray<P>]) =>
...
/** @internal */
export const lensAtKey =
  (key: string) =>
  <S, A>(sa: Lens<S, RR.ReadonlyRecord<string, A>>): Lens<S, O.Option<A>> =>
...
export const prismAsOptional = <S, A>(sa: Prism<S, A>): Optional<S, A> =>
  optional(sa.getOption, a => prismSet(a)(sa));
...
/** @internal */
export const optionalAsTraversal = <S, A>(
  sa: Optional<S, A>,
): Traversal<S, A> =>
  traversal(
    <F>(F: Applicative<F>) =>
...
export const optionalComposeOptional =
  ...
  <S>(sa: Optional<S, A>): Optional<S, B> =>
    optional(flow(sa.getOption, O.chain(ab.getOption)), b =>
      optionalModify(ab.set(b))(sa),
    );

/** @internal */
export const optionalIndex =
  (i: number) =>
  <S, A>(sa: Optional<S, ReadonlyArray<A>>): Optional<S, A> =>
    pipe(sa, optionalComposeOptional(indexReadonlyArray<A>().index(i)));

/** @internal */
export const optionalIndexNonEmpty =
  (i: number) =>
  <S, A>(sa: Optional<S, RNEA.ReadonlyNonEmptyArray<A>>): Optional<S, A> =>
    pipe(sa, optionalComposeOptional(indexReadonlyNonEmptyArray<A>().index(i)));

/** @internal */
export const optionalKey =
  (key: string) =>
  <S, A>(sa: Optional<S, RR.ReadonlyRecord<string, A>>): Optional<S, A> =>
    pipe(sa, optionalComposeOptional(indexReadonlyRecord<A>().index(key)));

/** @internal */
export const optionalFindFirst = <A>(
...
/** @internal */
export const optionalFindFirstNonEmpty = <A>(
  predicate: Predicate<A>,
): Optional<RNEA.ReadonlyNonEmptyArray<A>, A> =>
  optional<RNEA.ReadonlyNonEmptyArray<A>, A>(
    RA.findFirst(predicate),
    a => as =>
      pipe(
  ...
  return <A>() =>
    traversal(<F>(F: Applicative<F>) => {
      ...
      const traverseF: <A, B>(
        ta: HKT<T, A>,
        f: (a: A) => HKT<F, B>,
      ) => HKT<F, HKT<T, B>> = isIdentity(F) ? (T.map as any) : T.traverse(F);
...
/** @internal */
export const index = <S, I, A>(
  index: Index<S, I, A>["index"],
): Index<S, I, A> => ({ index });

/** @internal */
export const indexReadonlyArray = <A = never>(): Index<
  ReadonlyArray<A>,
  number,
  A
> =>
...
/** @internal */
export const indexReadonlyRecord = <A = never>(): Index<
  RR.ReadonlyRecord<string, A>,
  string,
  A
> =>
  index(k =>
...
/** @internal */
export function atReadonlyRecord<A = never>(): At<
  RR.ReadonlyRecord<string, A>,
  string,
  O.Option<A>
> {
