...
export const anon =
  <A>(a: A) =>
  (pred: Predicate<A>): Iso<Option<A>, A> => ({
    get: getOrElse(() => a),
...
export const composeLens = <A, B>(
  ab: Lens<A, B>,
): (<S>(sa: Iso<S, A>) => Lens<S, B>) => flow(asLens, _.lensComposeLens(ab));
...
export const prop = <A, P extends keyof A>(
  prop: P,
): (<S>(sa: Iso<S, A>) => Lens<S, A[P]>) => flow(asLens, _.lensProp(prop));
...
export const index = (
  i: number,
): (<S, A>(sa: Iso<S, ReadonlyArray<A>>) => Optional<S, A>) =>
...
export const key = (
  key: string,
): (<S, A>(sa: Iso<S, ReadonlyRecord<string, A>>) => Optional<S, A>) =>
...
export const atKey = (
  key: string,
): (<S, A>(sa: Iso<S, ReadonlyRecord<string, A>>) => Lens<S, Option<A>>) =>
 ...
 * @since 2.3.8
 */
export function traverse<T extends URIS>(
  T: Traversable1<T>,
...
export function findFirst<A>(
  predicate: Predicate<A>,
): <S>(sa: Iso<S, ReadonlyArray<A>>) => Optional<S, A>;
export function findFirst<A>(
  predicate: Predicate<A>,
): <S>(sa: Iso<S, ReadonlyArray<A>>) => Optional<S, A> {
...
export const imap: <A, B>(
  f: (a: A) => B,
  g: (b: B) => A,
) => <S>(sa: Iso<S, A>) => Iso<S, B> = (f, g) => ea => imap_(ea, f, g);
...
const imap_: Invariant2<URI>["imap"] = (ea, ab, ba) =>
  iso(flow(ea.get, ab), flow(ba, ea.reverseGet));
